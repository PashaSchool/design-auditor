import { parseRGB, normalizeColor, rgbToLab, rgbToHex, isNeutral, isTransparent, isNearWhite, isNearBlack, clusterColors } from '../utils/color.js';
...
export async function extractColors(page) {
    ...
    for (const item of raw.collected) {
        ...
        if (!entry.tags.includes(item.tag))
    ...
    const brandColors = clusterColors(all.filter(c => !isNeutral(c.rgb) && !isNearWhite(c.rgb) && !isNearBlack(c.rgb)), 8);
    ...
    for (const item of raw.collected) {
        ...
        if (isNearWhite(textRgb) && isNearWhite(bgRgb))
            continue;
        // skip if both are near-black
        if (isNearBlack(textRgb) && isNearBlack(bgRgb))
            continue;
