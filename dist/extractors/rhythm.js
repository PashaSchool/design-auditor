...
export async function extractRhythm(page) {
    const raw = await page.evaluate(({ textTags, skipTags }) => {
        ...
        elements.forEach(el => {
            ...
            if (!isNaN(mb) && mb > 0)
                margins.push({ value: Math.round(mb), property: 'margin-bottom', tag });
            // padding top/bottom
            const pt = parseFloat(style.paddingTop);
            const pb = parseFloat(style.paddingBottom);
            if (!isNaN(pt) && pt > 0)
                paddings.push({ value: Math.round(pt), property: 'padding-top', tag });
            if (!isNaN(pb) && pb > 0)
                paddings.push({ value: Math.round(pb), property: 'padding-bottom', tag });
        });
