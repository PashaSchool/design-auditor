export async function extractHeadings(page) {
    const raw = await page.evaluate(() => {
        ...
        return els.map((el, index) => {
            ...
            return { level, tag: el.tagName.toLowerCase(), text, fontSize, fontWeight: style.fontWeight, domIndex: index };
        });
    });
    const headings = raw;
    // ─── H1 count ────────────────────────────────────────────────────────────────
    ...
    return { headings, h1Count, skippedLevels, sizeInversions, levelSizes, hasLogicalOrder };
}
