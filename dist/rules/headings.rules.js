export function checkHeadings(data) {
    const violations = [];
    const { headings, h1Count, skippedLevels, sizeInversions, levelSizes, hasLogicalOrder } = data;
    ...
    if (sizeInversions.length > 0) {
        const examples = sizeInversions
            .slice(0, 2)
            .map(({ a, b }) => {
            const avgA = levelSizes[a.level].reduce((s, v) => s + v, 0) / levelSizes[a.level].length;
            const avgB = levelSizes[b.level].reduce((s, v) => s + v, 0) / levelSizes[b.level].length;
