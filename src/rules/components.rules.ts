...
export function checkComponents(data: ComponentsData): Violation[] {
  ...
  // ─── 1. Touch Targets ────────────────────────────────────────────────────────
  if (buttons.length > 0) {
    const tooSmall = buttons.filter(
      (b) => b.height < MIN_TOUCH_TARGET && b.height > 0
    );
  ...
  if (buttons.length >= 2) {
    const paddingVariants = new Set(
      buttons.map(
        (b) =>
          `${b.paddingTop}/${b.paddingRight}/${b.paddingBottom}/${b.paddingLeft}`
      )
    );
  ...
  const magicNumbers = data.zIndices.filter(
    (z) => z > 1000 && z !== 9999 && z % 100 !== 0
  );
