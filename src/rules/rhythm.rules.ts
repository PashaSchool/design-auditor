import { RhythmData } from '@extractors/rhythm.js';
...
function scoreMultiples(values: number[], base: number): number {
...
function findOutliers(values: number[], bases: number[]): number[] {
  return [
    ...new Set(
      values.filter((v) => !bases.some((base) => isMultipleOf(v, base)))
    ),
  ].sort((a, b) => a - b);
}

// Detects the most likely base grid of the site (4 or 8px)
function detectGrid(values: number[]): 4 | 8 {
...
export function checkRhythm(data: RhythmData): Violation[] {
  ...
  if (!isRhythmUnitClean) {
    violations.push({
      ...
      hint:
        `body: font-size ${bodyFontSize}px × line-height ${bodyLineHeight.toFixed(2)} = ${rhythmUnit}px. ` +
  ...
  const lhOutliers = findOutliers(data.uniqueLineHeights, [
    rhythmUnit,
    rhythmUnit / 2,
  ]);
