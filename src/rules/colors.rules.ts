import { ColorsData } from '@extractors/colors.js';
...
export function checkColors(data: ColorsData): Violation[] {
  ...
  const mergeCandidates: Array<[string, string, number]> = [];
  ...
  if (data.brandColors.length >= 3) {
    const totalBrandUsage = data.brandColors.reduce(
      (s, c) => s + c.totalCount,
      0
    );
    const [p, s, a] = data.brandColors.map(
      (c) => (c.totalCount / totalBrandUsage) * 100
    );
  ...
  const uniqueFailing = failing.filter(
    (p, i, arr) =>
      arr.findIndex((x) => x.textHex === p.textHex && x.bgHex === p.bgHex) === i
  );
  ...
  if (uniqueFailing.length > 5) {
    ...
    const examples = uniqueFailing
      .map(
        (p) =>
          `${p.textHex} on ${p.bgHex} (${p.contrast}:1 — needs ${p.fontSize >= 18 ? '3' : '4.5'}:1)`
      )
      .join(', ');
    violations.push({
      id: 'contrast-warn',
