...
export interface ShadowEntry {
  ...
  offsetY: number     // vertical offset (light direction)
  blur: number
  spread: number
  count: number       // how many elements have this shadow
  tags: string[]
}
...
export interface ComponentsData {
  ...
  // Border radius
  uniqueRadii: string[]               // all unique values
  radiiByElement: Record<string, string[]> // { 'button': ['4px','8px'], ... }
...
export async function extractComponents(page: Page): Promise<ComponentsData> {
  const raw = await page.evaluate(() => {
    // ─── Buttons ────────────────────────────────────────────────────────────────
    const buttonEls = Array.from(
      document.querySelectorAll('button, [role="button"], input[type="submit"], input[type="button"], a.btn, a[class*="button"]')
    )
    ...
    const radiiMap: Record<string, Set<string>> = {}
    ...
    allEls.forEach(el => {
      ...
      if (!r || r === '0px' || r === '0px 0px 0px 0px') return
      if (['script','style','meta','head','link'].includes(tag)) return

      if (!radiiMap[tag]) radiiMap[tag] = new Set()
      radiiMap[tag].add(r)
    })

    const radiiByElement: Record<string, string[]> = {}
    ...
    // ─── Shadows ─────────────────────────────────────────────────────────────────
    const shadowMap = new Map<string, { count: number; tags: Set<string>; offsetY: number; blur: number; spread: number }>()
