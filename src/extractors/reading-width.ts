...
export async function extractReadingWidth(page: Page): Promise<ReadingWidthData> {
  const blocks = await page.evaluate(({ tags, ratio }) => {
    const results: Array<{
      tag: string
      widthPx: number
      fontSize: number
  ...
  const avgCharCount = blocks.length > 0
