...
const TEXT_TAGS = ['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li', 'span', 'a', 'label', 'td', 'th']
const SKIP_TAGS = ['script', 'style', 'meta', 'head', 'link', 'br', 'hr', 'img', 'svg', 'path']

export async function extractRhythm(page: Page): Promise<RhythmData> {
  const raw = await page.evaluate(({ textTags, skipTags }) => {
    // --- body base values ---
    const bodyStyle = window.getComputedStyle(document.body)
    const bodyFontSize = parseFloat(bodyStyle.fontSize) || 16
    const rawLineHeight = bodyStyle.lineHeight
    const bodyLineHeightPx = rawLineHeight === 'normal'
      ? bodyFontSize * 1.5
    ...
    const lineHeights: Array<{ value: number; property: string; tag: string }> = []
    ...
    elements.forEach(el => {
      ...
      if (textTags.includes(tag) && fontSize > 0) {
        ...
        if (!isNaN(lhPx) && lhPx > 0) {
          lineHeights.push({ value: Math.round(lhPx), property: 'line-height', tag })
        }
      }
      ...
      if (!isNaN(mt) && mt > 0) margins.push({ value: Math.round(mt), property: 'margin-top', tag })
      if (!isNaN(mb) && mb > 0) margins.push({ value: Math.round(mb), property: 'margin-bottom', tag })

      // padding top/bottom
      const pt = parseFloat(style.paddingTop)
      const pb = parseFloat(style.paddingBottom)
      if (!isNaN(pt) && pt > 0) paddings.push({ value: Math.round(pt), property: 'padding-top', tag })
      if (!isNaN(pb) && pb > 0) paddings.push({ value: Math.round(pb), property: 'padding-bottom', tag })
    })
  ...
  const uniqueLineHeights = [...new Set(raw.lineHeights.map(v => v.value))].sort((a, b) => a - b)
  const uniqueMargins = [...new Set(raw.margins.map(v => v.value))].sort((a, b) => a - b)
  const uniquePaddings = [...new Set(raw.paddings.map(v => v.value))].sort((a, b) => a - b)
